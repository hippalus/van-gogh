use image::{buffer::ConvertBuffer, GrayImage, ImageBuffer, imageops::huerotate, Rgba};

use self::bridge::*;

type ImageRgbaRef<'b> = ImageBuffer<Rgba<u8>, &'b [u8]>;

fn create_image_ref<'b>(buf: &'b [u8], width: u32, height: u32) -> RbImageRgbaRef<'b> {
    RbImageRgbaRef {
        data: buf.as_ptr(),
        data_len: buf.len(),
        width,
...
impl<'b> From<&RbImageRgbaRef<'b>> for ImageRgbaRef<'b> {
    fn from(img: &RbImageRgbaRef<'b>) -> Self {
        ...
        ImageBuffer::from_raw(img.width, img.height, buffer).expect("Invalid image data")
    }
}
